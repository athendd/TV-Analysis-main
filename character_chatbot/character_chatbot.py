...
class CharacterChatBot():
    def __init__(self, model_path, data_path = '/content/data/naruto.csv', huggingface_token = None):
        ...
        #Get model if it exists on huggingface
    ...
    def load_model(self, model_path):
        ...
        pipeline = transformers.pipeline('text-generation', model = model_path, model_kwargs = {'torch_dtype': torch.float16, 'quantization_config': bnb_config})
        
        return pipeline
            
    def train(self, base_model_name_or_path, dataset, output_dir = './results', per_device_train_batch_size = 1, 
              per_device_eval_batch_size = 1, gradient_accumalation_steps = 1, optim = 'paged_adamw_32bit', save_steps = 200, 
              logging_steps = 10, learning_rate = 2e-4, max_grad_norm = 0.3, max_steps = 300,
              warmup_ratio = 0.3, lr_scheduler_type = 'contant'):
        ...
        base_model = AutoModelForCausalLM.from_pretrained(base_model_name_or_path, return_dict = True, quantization_config = bnb_config,
